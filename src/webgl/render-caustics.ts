...
import type { ElementState, GlassVariantDef } from '../types/typedefs.js';
 ...
 */
export function _renderCausticsGL(es: ElementState, now: number) {
    const gl  = _state.glBackend;
    const uni = _state.glUniforms;
    const glCanvas = _state.glCanvas;
    if (!gl || !_state.glProgram || !glCanvas) return;
    ...
    const GLASS_TYPE_MAP: Record<string, number> = { BK7: 0, SF11: 1, NK51A: 2, NBK10: 3, F2: 4 };
    ...
    const vd = (GLASS_VARIANTS as Record<string, GlassVariantDef>)[variantKey] ?? GLASS_VARIANTS.clear;
